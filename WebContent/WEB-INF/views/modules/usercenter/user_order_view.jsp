      ...
      <div class="box-hd">
        ...
        <div class="more clearfix">
          ...
          <div class="actions">
            ...
            <c:if test="${orderVO.orderStatus eq 5}"> <a title="申请售后" href="" class="btn btn-small btn-line-gray" data-order-id="${orderVO.orderNumber}">申请售后</a> </c:if>
           	<c:if test="${orderVO.orderStatus eq 4 }"><a class="btn btn-small btn-primary" href="${ctx}/buy/takedelivery/${orderVO.orderNumber}" target="_blank">收货</a></c:if>
          </div>
        </div>
      </div>
      <div class="box-bd">
          ...
          <div class="order-detail">
            <div class="order-summary">
              <div class="order-status">
                <c:if test="${orderVO.orderStatus eq 1}">待付款</c:if>
                <c:if test="${orderVO.orderStatus eq 2 }">待出库</c:if>
                <c:if test="${orderVO.orderStatus eq 3}">待揽件</c:if>
                <c:if test="${orderVO.orderStatus eq 4}">运送中</c:if>
                <c:if test="${orderVO.orderStatus eq 5}">已收货</c:if>
                <c:if test="${orderVO.orderStatus eq 11}">交易完成</c:if>
                <c:if test="${orderVO.orderStatus eq 12}">订单取消</c:if>
              </div>
              <div class="order-progress">
                <ol class="progress-list clearfix">
                  ...
                  <li class="step ${status == true ? 'step-done':''}">
                    <div><span class="text">揽件</span></div>
                    <div class="info">
                      <fmt:formatDate value="${info}" pattern="yyyy年MM月dd日 hh:mm" />
                    </div>
                  </li>
                  ...
                  <li class="step ${status == true ? 'step-done':''}">
                    <div><span class="text">运送</span></div>
                    <div class="info">
                      <fmt:formatDate value="${info}" pattern="yyyy年MM月dd日 hh:mm" />
                    </div>
                  </li>
          ...
          <div id="editAddr" class="order-detail-info">
            ...
            <table class="info-table">
              <tbody>
                ...
                <tr>
                  <th>收货地址：</th>
                  <td>&nbsp;${orderVO.orderShipment.userAdress}</td>
                </tr>
              </tbody>
            </table>
            <div class="actions">
