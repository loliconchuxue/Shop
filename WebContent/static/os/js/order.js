	...
	$('#J_cancelOrder').on("click", function() {
		...
		layer.confirm('你真的要取消此订单吗?', {
			...
			$.ajax({
				type : 'post',
				dataType : 'json',
				url : baselocation + '/uc/order/cancelOrder',
				data : {
				...
				success : function(result) {
					if (result.code == 1) {
						...
						location.href=baselocation +"/uc/order/list"; //刷新当前页面.
					} else {
...
$(function() {
	$("#J_submitEditTime").on("click", function() {
		/*var params = {};*/
		var shipmentTime = $(".time-list").find(".active").attr('data-value');
		var orderNumber = $(".J_editTime").attr('data-order-id');
		$.ajax({
			url : baselocation + '/uc/order/time',
			type : 'post',
			dataType : 'json',
			data : {
 ...
 */
function saveAddr(a) {
	var orderShipmentId = $(".J_editAddr").attr('data-shipment-id');
	var orderId = $(".J_editAddr").attr('data-order-id');
	$.ajax({
		type : "post",
		url : baselocation + '/uc/order/shipment',
		data : {
			'userName':a.userName,
			'userPhone':a.userPhone,
			'userAdress':a.userAdress,
