				...
				$("#verifyYz").click(function() {
					...
					$.ajax({
						...
						success : function(result) {
							if (result.code == 1) {
								$("#verifyYz").attr("data-emailSign", result.message);
...
$(function() {
	...
	$("#btn_part1").click(function() {
		...
		$.ajax({
			type : "POST",
			url : baselocation + '/user/register',
			data : params,
			dataType : "json",
			success : function(result) {
				if (result.code == 1) {
...
$(function() {
	...
	$("#btn_Pswpart1").click(function() {
		...
		$.ajax({
			type : "POST",
			url : baselocation + '/user/forgetPassword',
			data : {
			...
			success : function(result) {
				if (result.code == 1) {
					...
					$(".part2").children(".item").children(".c-blue").text(result.message);
					$(".message").hide();
				} else {
					$(".message").show();
	...
	$("#btn_Pswpart2").click(function() {
		...
		$.ajax({
			type : "POST",
			url : baselocation + '/user/verifyEmail',
			data : {
				captcha : captcha,
				email : email,
				emailSign : emailSign,
	...
	$("#btn_Pswpart3").click(function() {
		...
		$.ajax({
			type : "POST",
			url : baselocation + '/user/resetPassword',
			data : {
				loginPassword : loginPassword,
				captcha : captcha,
				email : email,
			...
			success : function(result) {
				if (result.code == 1) {
					$(".part4").children("h3").text("恭喜,您已成功找回密码,现在开始您的购物之旅吧!");
					$(".part3").hide();
					$(".part4").show();
					$(".step li").eq(2).addClass("on");
					countdown({
...
$(function() {
	$('#kaptchaImage').click(function() {
		$(this).attr('src', baselocation + '/user/code-image.jpg?' + Math.floor(Math.random() * 100));
	})
});
